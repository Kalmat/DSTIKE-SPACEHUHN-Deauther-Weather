...
void DisplayUI::setup() {
    ...
    createMenu(&clockMenu, &mainMenu, [this]() {
        ...
        addMenuNode(&clockMenu, D_CLOCK_UPDATE, [this]() { // CLOCK UPDATE TIME ON INTERNET
            mode = DISPLAY_MODE::CLOCK_DISPLAY;
            forceUpdate = true;
            forceDraw = true;
            //display.setFont(ArialMT_Plain_24);
...
void DisplayUI::update(bool force) {
    ...
    if (currentTime > nextTimeout || (tempOff && buttonTime >= currentTime - timeout)) {
        if (!tempOff) {
...
void DisplayUI::setupButtons() {
    ...
    up->setOnClicked([this]() {
        ...
        if (!tempOff) {
            ...
            } else if (mode == DISPLAY_MODE::CLOCK_DISPLAY) { // when in clock display, change visible frame
                weatherUP();
    ...
    up->setOnHolding([this]() {
        ...
        if (!tempOff) {
            if (mode == DISPLAY_MODE::MENU) {                 // when in menu, go up or down with cursor
                ...
                scan.setChannel(wifi_channel + 1);
            } else if (mode == DISPLAY_MODE::CLOCK_DISPLAY) { // when in clock display, change visible frame and stop animation
                weatherRotate();
            } else if (mode == DISPLAY_MODE::CLOCK) {         // when in clock, change time
    ...
    down->setOnClicked([this]() {
        ...
        if (!tempOff) {
            if (mode == DISPLAY_MODE::MENU) {                 // when in menu, go up or down with cursor
                ...
                scan.setChannel(wifi_channel - 1);
            } else if (mode == DISPLAY_MODE::CLOCK_DISPLAY) { // when in clock display, show previous frame and stop animation
                weatherDOWN();
            } else if (mode == DISPLAY_MODE::CLOCK) {         // when in clock, change time
                setTime(clockHour, clockMinute - 1, clockSecond);
            }
    ...
    down->setOnHolding([this]() {
        ...
        if (!tempOff) {
            if (mode == DISPLAY_MODE::MENU) {                 // when in menu, go up or down with cursor
                ...
                scan.setChannel(wifi_channel - 1);
            } else if (mode == DISPLAY_MODE::CLOCK_DISPLAY) { // when in clock display, restore frame animation
                weatherRotate();
            } else if (mode == DISPLAY_MODE::CLOCK) {           // when in clock, change time
                setTime(clockHour, clockMinute - 10, clockSecond);
            }
    ...
    if (force || ((currentTime - drawTime > drawInterval) && currentMenu)) {
        ...
        switch (mode) {
            ...
            case DISPLAY_MODE::CLOCK:
                ...
                break;
            case DISPLAY_MODE::CLOCK_DISPLAY:
                drawClock(forceDraw, forceUpdate);
                forceDraw = false;
                forceUpdate = false;
                break;
...
void DisplayUI::drawClock(bool forceDraw, bool forceUpdate) {
    drawClockWeather(&display, forceDraw, forceUpdate);
}

void DisplayUI::drawSetClock() {
    drawSetClockW(&display);
}
