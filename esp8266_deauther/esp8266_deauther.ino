...
void setup() {
    ...
    if (settings::getDisplaySettings().enabled) {
        displayUI.setup();
        timeout = settings::getDisplaySettings().timeout * 1000;
        nextTimeout = timeout;
        // displayUI.mode = DISPLAY_MODE::INTRO;       // Original start showing menu
        displayUI.mode = DISPLAY_MODE::CLOCK_DISPLAY;  // Set this mode to start on Clock/Weather mode (AP Scan and other startup tasks are also performed)
    }
...
void loop() {
    ...
    // delay(100);  // This saves CPU and battery, but not sure if it can interfere with some other tasks --> CONFIRM!!!
}
